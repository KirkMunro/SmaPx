...
function Get-SmaPxRunbookVersion {
    [CmdletBinding(DefaultParameterSetName='ByName')]
    [OutputType('Microsoft.SystemCenter.ServiceManagementAutomation.RunbookDefinition')]
    param(
        ...
        [Parameter(Mandatory=$true, ParameterSetName='ByVersionId')]
        [ValidateNotNullOrEmpty()]
        [System.Guid[]]
        $VersionId,
        ...
        [ValidateNotNullOrEmpty()]
        [ValidateSet('Published','Draft','All')]
        [System.String]
        $Type = 'All',
    ...
    process {
        try {
            switch ($PSCmdlet.ParameterSetName) {
                'ByVersionId' {
                    #region Lookup the runbook definition.

                    Get-SmaRunbookDefinition -VersionId $VersionId @connectionParameters
                ...
                default {
                    if ($_.PSTypeNames -contains 'Orchestrator.WebService.Client.OaaSClient.Runbook') {
                        ...
                        foreach ($runbook in $runbooks) {
                            if ((@('All','Published') -contains $Type) -and
                                $runbook.PublishedRunbookVersionID) {
                                Get-SmaPxRunbookVersion -VersionId $runbook.PublishedRunbookVersionID @connectionParameters
                            }
                            if ((@('All','Draft') -contains $Type) -and
                                $runbook.DraftRunbookVersionID) {
                                Get-SmaPxRunbookVersion -VersionId $runbook.DraftRunbookVersionID @connectionParameters
                            }
                        }
...
Export-ModuleMember -Function Get-SmaPxRunbookVersion